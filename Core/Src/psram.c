  ...
  + Initialization steps:
     o Initialize the PSRAM external memory using the PSRAM_Init() function. This 
  ...
  + PSRAM read/write operations
     o PSRAM external memory can be accessed with read/write operations once it is
       ...
       PSRAM_ReadData()/PSRAM_WriteData(), or by DMA transfer using the functions
       PSRAM_ReadData_DMA()/PSRAM_WriteData_DMA().
     ...
     o User can implement his own functions for read/write access with his desired 
       configurations.
     o If interrupt mode is used for DMA transfer, the function PSRAM_DMA_IRQHandler()
...
uint8_t PSRAM_Init(void)
{
  ...
  PSRAM_MspInit(&psramHandle, NULL); /* __weak function can be rewritten by the application */
...
{ 
  ...
  PSRAM_MspDeInit(&psramHandle, NULL);
...
/**
  ...
  * @retval None
  */
__weak void PSRAM_MspInit(SRAM_HandleTypeDef  *hsram, void *Params)
{  
  (void)Params; // Mark Params as intentionally unused.
  (void)hsram; // Mark hsram as intentionally unused. 
...
/**
  ...
  * @retval None
  */
__weak void PSRAM_MspDeInit(SRAM_HandleTypeDef  *hsram, void *Params)
{  
    (void)Params; // Mark Params as intentionally unused. 
    (void)hsram; // Mark hsram as intentionally unused.
